...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    String showUrl = "http://10.0.2.2/localisation_back/showPositions.php";
    ...
    private void startLocationUpdates() {
        // Configuration de la demande de localisation avec une haute précision
            ...
            public void onLocationResult(@NonNull LocationResult locationResult) {
                for (android.location.Location location : locationResult.getLocations()) {
                    if (location != null) {
                        ...
                        // Affichage dans le log pour débogage
                        Log.d("PositionDebug", "Nouvelle position : " + lat + ", " + lon);

                        // Mise à jour de l'interface utilisateur avec la position actuelle
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            // Si la permission est accordée, demander les mises à jour de localisation
            fusedLocationClient.requestLocationUpdates(locationRequest, locationCallback, Looper.getMainLooper());
        } else {
            // Sinon, demander la permission de localisation
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, 1);
        }
    }
    ...
    private void addPosition(final double lat, final double lon) {
        String insertUrl = "http://10.0.2.2/localisation_back/createPosition.php";

        StringRequest request = new StringRequest(Request.Method.POST, insertUrl, response -> {
            Log.d("MAP_DEBUG", "Réponse brute : " + response);
            try {
